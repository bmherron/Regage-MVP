...
getUserCommList=function(user){
   //console.log('getUserCommList')
   var userDefaultCommKey = "", userCommKey = "", commList = []
   if (!user){
     return commList;
   }
  
//   if (user.profile.status === 0){
//     userCommKey = 'state/US/NC';
//     commList.push(userCommKey);
//     return commList;
//   }
  
   if (Session.get('commLevel')){
     commLevel = Session.get('commLevel');
     if (commLevel == 'mycommunity'){
         if (user.profile.city && user.profile.state){     
           userCommKey = 'city/US/' + user.profile.state + '/' + user.profile.city ;
         }
     } 
     if (commLevel == 'myarea' || commLevel == 'mypeers'){
         if (user.profile.county && user.profile.state){     
           userCommKey = 'group/countiesregion/US/' + user.profile.state + '/' + user.profile.county ;
         }       
     } 
     if (commLevel == 'mystate'){
         if (user.profile.city && user.profile.state){     
           userCommKey = 'state/US/' + user.profile.state  ;
         }
     } 
       ...
       if (user.profile.defaultCommKey){     
         userCommKey =user.profile.defaultCommKey;
       }

   }


  if (userCommKey.substring(0,5)=='group'){
  ...
  //console.log(userCommKey)
  //console.log(commList)
  return commList;
};


getFollowedUserIdList=function(user){
   console.log('getFollowedUserIdList')
