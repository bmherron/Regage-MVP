...
Posts.deny({
  update: function(userId, post, fieldNames) {
    ...
    return (_.without(fieldNames, 'title', 'url', 'body', 'shortUrl', 'shortTitle', 'categories', 'tagLine', 'fileName', 'fundingGoal', 'fundingDuration','nonProfitDoc', 'fundingFlag',  'fundingTotal', 'fundingCount', 'fundingAt', 'fundingStatus', 'fundingAccount', 'bodyWho', 'bodyWhere', 'bodyWhat', 'nonProfit').length > 0);
  }
});
...
Meteor.methods({
  post: function(post){
    var title = cleanUp(post.title),
        body = post.body,
        bodyWho = post.bodyWho,
        bodyWhat = post.bodyWhat,
        bodyWhere = post.bodyWhere,
        commList = post.commList,
        userId = this.userId,
    ...
    if(postType == '2'){
      ...
      if(!post.bodyWho)
        throw new Meteor.Error(602, i18n.t('Please tell us about yourself.'));
      if(!post.bodyWhat)
        throw new Meteor.Error(602, i18n.t('Please provide your story.'));
      if(!post.bodyWho)
        throw new Meteor.Error(602, i18n.t('Please tell us where the money will go.'));
      if(!post.fundingGoal)
        throw new Meteor.Error(602, i18n.t('Please fill out all fields'));
       if(!post.fundingDuration)
        throw new Meteor.Error(602, i18n.t('Please fill out all fields'));
    ...
    //Endorsements
    properties.endorsements = [{userId: userId, type: 'create'}];
  ...
  post_endorse: function(postId){
    //Endorsements
    Posts.update(postId, {
      $addToSet:  { endorsements: {userId: Meteor.userId(), type: 'endorse'}}
    });
  },   
});
